...
impl Message {
    ...
    pub fn signed(mut self, key: &mut SigningKey) -> String {
        let out = self.serialize();
        self = self.with_header("signature".to_owned(), key.sign(out.as_bytes()).to_vec());
        self.serialize()
    }
    ...
    pub fn encrypt_body(mut self, target_pub_key: &VerifyingKey) -> Result<Self, FLESHDecodeError> {
        // Ensure a target is set
        if self.target.is_none() {
            return Err(FLESHDecodeError::NoTargetProvided);
        }
        ...
        let cipher = ChaCha20Poly1305::new_from_slice(key).map_err(|_| FLESHDecodeError::InvalidKey)?;
        ...
        let encrypted_body = cipher.encrypt(nonce, self.body.as_ref()).map_err(|_| FLESHDecodeError::EncryptionFailed)?;
    ...
    pub fn decrypt_body(mut self, my_signing_key: &SigningKey) -> Result<Self, FLESHDecodeError> {
        // Retrieve the ephemeral key and nonce from headers
        let ephemeral_key_bytes = self.headers.get("ephemeral-key").ok_or(FLESHDecodeError::MissingEphemeralKey)?;
        let nonce_bytes = self.headers.get("nonce").ok_or(FLESHDecodeError::MissingNonce)?;

        // Convert bytes to array for X25519PublicKey::from
        let ephemeral_key_array: [u8; 32] =
            ephemeral_key_bytes.as_slice().try_into().map_err(|_| FLESHDecodeError::InvalidFormat)?;
        ...
        let my_secret_bytes: [u8; 32] = my_signing_key.to_bytes();
        ...
        let decrypted_body = cipher.decrypt(nonce, self.body.as_ref()).map_err(|_| FLESHDecodeError::DecryptionFailed)?;

        self.body = decrypted_body;
        Ok(self)
    }

    pub fn verify(&self, key: &VerifyingKey) -> Result<(), FLESHDecodeError> {
        let signature_b64 = self.headers.get("signature").ok_or(FLESHDecodeError::InvalidSignature)?;
        let signature_bytes = general_purpose::STANDARD.decode(signature_b64)?;

        let signature = Signature::from_bytes(&signature_bytes.try_into().map_err(|_| FLESHDecodeError::InvalidFormat)?);
        let message = self.serialize_for_signing();

        key.verify_strict(message.as_bytes(), &signature).map_err(|_| FLESHDecodeError::InvalidSignature)?;
        Ok(())
    }

    pub fn deserialize(s: &str, me: (Fluid, SigningKey)) -> Result<Self, FLESHDecodeError> {
        let mut lines = s.lines();

        let first_line = lines.next().ok_or(FLESHDecodeError::InvalidFormat)?;
        let mut parts = first_line.splitn(3, ' ');
        let version_str = parts.next().ok_or(FLESHDecodeError::InvalidFormat)?;
        let status_str = parts.next().ok_or(FLESHDecodeError::InvalidFormat)?;

        let target = parts.next();
        if let Some(target) = target
            && target != me.0.to_string()
        {
            return Err(FLESHDecodeError::NotRecipient);
        }

        let version = version_str.parse::<u16>().map_err(|_| FLESHDecodeError::InvalidVersion(version_str.to_string()))?;
        let status = status_str.parse::<u16>().map_err(|_| FLESHDecodeError::InvalidStatus(status_str.to_string()))?;
        ...
        for line in lines {
            if in_headers {
                ...
                let key = header_parts.next().ok_or(FLESHDecodeError::InvalidFormat)?.to_string();
                let value_b64 = header_parts.next().ok_or(FLESHDecodeError::InvalidFormat)?;
        ...
        let deser = Self { version, status, headers, body, target: target.map(|_| me.0) };

        if target.is_some() {
            deser.verify(&me.1.verifying_key())?;
            deser.decrypt_body(&me.1)
        } else {
            Ok(deser)
        }
    }
